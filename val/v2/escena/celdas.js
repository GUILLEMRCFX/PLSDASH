...
/**
 * Alternativa a la teselación de Voronoi: una esfera geodésica.
 *
 * Un icosaedro subdividido y proyectado a la esfera. Frente al Voronoi, los
 * triángulos son casi todos iguales y el reparto es regular — se lee como una
 * estructura construida en vez de como algo orgánico. Está aquí para poder
 * comparar las dos mirándolas, no imaginándolas.
 *
 * `detalle` no son subdivisiones sucesivas: `IcosahedronGeometry` parte cada
 * arista del icosaedro en `detalle+1`, así que salen (detalle+1)² × 20
 * triángulos. De ahí, por Euler, V = T/2 + 2 y A = 3T/2.
 *
 * Devuelve exactamente la misma forma que `construirCeldas` para que la escena
 * pueda intercambiarlas sin saber cuál está montada.
 */
export function construirGeodesica(THREE, { detalle = 7, semilla = 20260815 } = {}) {
  const t0 = performance.now();
  const aleatorio = generador(semilla);

  const cascara = new THREE.IcosahedronGeometry(1, detalle);
  const pos = cascara.attributes.position.array;
  const nTri = pos.length / 9;

  // `IcosahedronGeometry` viene sin índices: cada triángulo repite sus tres
  // vértices. Se deduplican cuantizando, que además absorbe el ruido de coma
  // flotante entre triángulos vecinos.
  const idDe = new Map();
  const unicos = [];
  const triVert = new Int32Array(nTri * 3);
  for (let v = 0; v < nTri * 3; v++) {
    const o = v * 3;
    const clave = `${Math.round(pos[o] * 1e5)},${Math.round(pos[o + 1] * 1e5)},${Math.round(pos[o + 2] * 1e5)}`;
    let id = idDe.get(clave);
    if (id === undefined) {
      id = unicos.length / 3;
      unicos.push(pos[o], pos[o + 1], pos[o + 2]);
      idDe.set(clave, id);
    }
    triVert[v] = id;
  }
  const nUnicos = unicos.length / 3;

  // Aristas únicas: cada una la ven dos triángulos, así que se emiten una vez.
  const vistas = new Set();
  const aristas = [];
  const anotar = (a, b) => {
    const clave = a < b ? a * nUnicos + b : b * nUnicos + a;
    if (vistas.has(clave)) return;
    vistas.add(clave);
    aristas.push(unicos[a * 3], unicos[a * 3 + 1], unicos[a * 3 + 2],
                 unicos[b * 3], unicos[b * 3 + 1], unicos[b * 3 + 2]);
  };
  for (let t = 0; t < nTri; t++) {
    const a = triVert[t * 3], b = triVert[t * 3 + 1], c = triVert[t * 3 + 2];
    anotar(a, b); anotar(b, c); anotar(c, a);
  }

  // Un matiz por triángulo, plano en sus tres vértices, para que cada cara
  // tenga su tono sin degradados entre vecinas.
  const tonos = new Float32Array(nTri * 3);
  for (let t = 0; t < nTri; t++) {
    const v = aleatorio();
    tonos[t * 3] = v; tonos[t * 3 + 1] = v; tonos[t * 3 + 2] = v;
  }
  cascara.setAttribute('aTono', new THREE.BufferAttribute(tonos, 1));

  // Puntos de malla: uno por vértice. Aquí no hay centros de celda que añadir.
  const puntos = new Float32Array(unicos);
  const brilloPunto = new Float32Array(nUnicos);
  for (let i = 0; i < nUnicos; i++) brilloPunto[i] = 0.5 + aleatorio() * 0.5;

  return {
    cascara,
    aristas: new Float32Array(aristas),
    puntos, brilloPunto,
    // `vertices` solo lo tiene la geodésica: es lo que deja clavar los nodos
    // de validador en vértices reales de la malla en vez de flotando encima.
    vertices: puntos,
    nPuntos: nUnicos,
    nCeldas: nUnicos,
    nSegmentos: aristas.length / 6,
