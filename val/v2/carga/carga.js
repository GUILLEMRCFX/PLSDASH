/**
 * La pantalla de carga: el reloj.
 *
 * Este módulo es lo único que sabe CUÁNDO pasa cada cosa. La pantalla en sí
 * —`naciendo.js`— solo sabe dibujar.
 *
 ...
 * ## El mínimo
 *
 * 1600 ms de suelo. El motivo de que no sea más corto: por debajo de ~1,2 s la
 * animación se lee como un parpadeo y da la sensación contraria a la que se
 * busca —parece que algo ha fallado y se ha repintado—. Y de que no sea más
 * largo: esto se ve cada vez que abres el panel, y el panel se abre a diario.
 *
 ...
 * esfera quieta y se funde. El mínimo baja a 400 ms, porque esperar por algo
 * que no se mueve es esperar por nada. La espera por datos se respeta igual.
 */
 ...
 * Arranca la pantalla.
 *
 * `pieza` es el módulo de la pantalla. Tiene que exportar `montar(caja, ctx)` y
 * devolver `{ entrada(), salida(), destruir() }`, donde `entrada` y `salida`
 ...
 * Se usa así:
 *
 *   const carga = iniciarCarga(document.body, pieza, {
 *     alEntregar: esfera => esfera.mudar(hueco),   // opcional
 *   });
 *   const datos = await cargarTodo();
 *   await carga.terminar();       // espera al suelo, hace la salida y entrega
 *
...
export function iniciarCarga(caja, pieza, opciones = {}) {
  ...
  const vista = pieza.montar(caja, { ...opciones, reducido: menos });
  const entrada = Promise.resolve(vista.entrada());

  let fin = null;
  return {
    /** Para las pruebas, y para que quien llama recoja la esfera al final. */
    vista,
    minMs,
    terminar() {
      if (fin) return fin;
      fin = (async () => {
        ...
        // 3. La salida, entera.
        await vista.salida();
        /* 4. La entrega, ENTRE la salida y el desmontaje. El orden importa: la
              esfera hay que mudarla al hueco de la pestaña mientras su lienzo
              sigue vivo dentro de la capa, no después de quitarla. */
        if (opciones.alEntregar) await opciones.alEntregar(vista.entregar?.());
        vista.destruir();
      })();
      return fin;
    },
