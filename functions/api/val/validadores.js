 ...
 *   lo pide el panel v1 en `val/index.html`, así:
 *
 *       const API = '/api/val';
 *       api('/validadores')        →  fetch(API + '/validadores')
 *
 *   El literal «api/val/validadores» NO APARECE en ningún fichero del
 *   repositorio, así que un grep del literal no lo encuentra jamás. Antes de
 *   volver a darlo por muerto, búscalo por el sufijo —`'/validadores'`— o pasa
 *   `rutas-test.js`, que reconstruye las URL como las reconstruye el navegador.
 *
 *   De aquí sale la línea «tu media: X %» bajo la casilla de Efectividad.
 *
